...
配额结果按模型或资源分组，完整展示接口实际返回的短周期、日、周、月窗口，包括已用、剩余、总量、百分比、重置时间和倒计时；不限量及“不在当前套餐”也会单独标记。账户明细不再截断。
...
| Kimi 官方 API | 可查询 | 可用、现金与赠金余额；域名跟随所选 OpenAI 兼容提供商 |
| Kimi Coding Plan | 可查询 | 周额度、滚动短周期、各窗口重置时间、加量包余额及月付费上限 |
| MiniMax Token Plan（国内） | 可查询 | 官方 CLI 同款的各模型当前周期、周额度、剩余比例、倍率与重置倒计时 |
| MiniMax Token Plan（海外） | 可查询 | 同上，使用海外区域端点 |
| MiniMax 官方 API（按量） | 仅控制台可查 | 官方未公开模型 API Key 余额接口 |
| LongCat | 仅控制台可查 | PAYG 与多个 30 天 Token Pack 依赖网页登录会话 |
| OpenCode | 仅控制台可查 | Workspace 账单依赖登录控制台/RPC |
| 火山引擎 Coding Plan | 仅控制台可查 | 官方未公开 API Key 配额查询接口 |
| 小米 MiMo API | 仅控制台可查 | 按量余额接口依赖小米网页登录会话 |
| 小米 Token Plan | 仅控制台可查 | Credits 套餐明细依赖小米网页登录会话 |

“仅控制台可查”不是网络错误：这些平台的模型 API Key 只能用于推理，不能认证官网账单接口。插件不会把模型密钥发送到推测的控制台路径，也不会要求或保存浏览器 Cookie。
...
- 对官方未公开 API-Key 账单端点的平台，页面会显示“仅控制台可查”及具体原因，不会伪造成功结果。
