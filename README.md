...
同一服务配置多个 Key 时，概览默认收起密钥明细，并按配额类目汇总所有查询成功且属于独立密钥的窗口：百分比直接相加（4 个满额 Key 显示 400%），次数、总额、已用和剩余数量分别求和；只要任一有效 Key 的对应窗口不限量，汇总窗口即显示不限量。失败 Key 不参与配额统计。账户级或无法确认归属范围的旧接口额度不会重复相加。完整账户明细不截断，可从卡片右侧平滑展开和收起。
