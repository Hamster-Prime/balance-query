...
也可以在“概览”顶部使用“自主查询”：选择查询项并输入 API Key 即可直接查看结果；Sub2API 和 New API 会额外要求服务地址。自主查询不依赖已保存的提供商映射，不读写余额缓存，输入的 Key 不会写入插件配置或浏览器存储。

Sub2API、New API 等自建实例直接使用所选 AI 提供商中的服务地址和密钥，无需在插件配置中重复填写。相同大类下按服务地址分组；同一服务只有一个 API Key 时直接显示账户卡片，配置多个 Key 时合并为默认收起的服务分组，组头展示查询状态及可安全汇总的余额和配额。查询优先使用每条密钥自己的代理地址，未单独设置时继承 CPA 全局代理，并支持 `direct` / `none` 显式直连。已停用的提供商或密钥不会发起查询。
...
概览和查询设置都按 OpenAI 兼容、Claude、xAI、Codex、Gemini 提供单选标签，一次只展示当前选择的大类；设置页会保留尚未保存的类目修改。配额结果按模型或资源分组，完整展示接口实际返回的短周期、日、周、月窗口，包括剩余、总量、百分比、重置时间和倒计时；进度条以剩余额度为宽度，并随额度降低由绿色转为黄色、红色。不限量及“不在当前套餐”也会单独标记。

同一服务配置多个 Key 时，概览默认收起密钥明细，并按稳定的配额标识、单位和归属范围分别汇总。纯百分比窗口显示“合计剩余 / 合计容量”（4 个满额 Key 显示 `400% / 400%`）；次数或金额窗口按总容量加权计算进度，不再对百分比做简单平均。明确不限量、套餐不包含、接口未返回、数据不完整和查询失败会分别计数，缺少某个周/月窗口的 Key 不会再从聚合结果中静默消失。账户级或无法确认归属范围的额度不会直接相加，数值不同时显示范围。

展开密钥后，每个 Key 使用带边框和状态侧边线的独立面板，正常、额度偏低和查询失败均保持一致的布局。完整账户明细不截断，可从卡片右侧平滑展开和收起。

查询结果会同时使用插件进程内缓存和当前浏览器会话中的脱敏快照。重新进入页面时会先恢复上次可展示结果，再在后台同步提供商配置；缓存仍有效时，即使结果包含部分数据或失败项也不会自动重复查询。缓存过期或设为 `0` 时仍会保留上次结果避免空白等待，同时在后台更新；点击“刷新余额”会绕过缓存重新查询。相同账户的并发查询会合并，刷新失败也不会覆盖当前可见结果。

设置保存前会合并 CPA 中的最新插件配置，避免其他页面新增的映射被整表覆盖；写入后还会等待插件运行时确认配置已经生效，再允许按新映射查询。保存期间继续编辑不会丢失后续草稿。

查询失败会区分认证、权限、余额或额度、限流、配置或接口、代理、DNS、TLS、网络、响应格式和上游服务等原因，并给出对应提示与处理建议。失败结果只会按类别短暂缓存，且不会超过配置的缓存时长，以避免重进页面时反复请求无效密钥或故障接口。
...
| New API | 可查询 | 密钥总量/已用/剩余、不限量、模型白名单、到期时间；不限量密钥仍展示真实已用额度，按站点 USD/CNY/令牌/自定义单位换算，兼容旧版 billing 接口 |
...
| MiniMax Token Plan（海外） | 可查询 | 同上，使用官方 `.io/v1/token_plan/remains` 端点，并兼容 Bearer 与 `x-api-key` 两种 CLI 鉴权方式 |
...
- 内存缓存键使用 API Key 的 SHA-256 摘要，不包含明文密钥；成功结果按配置时长缓存，失败结果仅按类别短暂缓存。
...
`make build` 默认读取本机 `GOOS`，构建产物分别为 Linux 的 `bin/balance-query.so`、macOS 的 `bin/balance-query.dylib` 或 Windows 的 `bin/balance-query.dll`。可以覆盖目标系统和输出路径，例如：

```bash
make build GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc OUTPUT=bin/balance-query.dll
```

由于共享库依赖 CGo，交叉编译不能只设置 `GOOS` / `GOARCH`，还需要安装目标平台对应的 C 编译工具链并正确设置 `CC`。推送 `v*` 标签后，GitHub Actions 会测试并构建 Linux、macOS、Windows 的 amd64/arm64 产物，同时更新插件商店注册表。
