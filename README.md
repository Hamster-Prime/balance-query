...
概览按 OpenAI 兼容、Claude、xAI、Codex、Gemini 提供单选标签，一次只展示当前选择的大类，切换后保留各服务的密钥展开状态。配额结果按模型或资源分组，完整展示接口实际返回的短周期、日、周、月窗口，包括剩余、总量、百分比、重置时间和倒计时；进度条以剩余额度为宽度，并随额度降低由绿色转为黄色、红色。不限量及“不在当前套餐”也会单独标记。
...
查询结果会同时使用插件进程内缓存和当前浏览器会话中的脱敏快照。账户、查询配置和管理会话未变化且缓存仍有效时，重新进入页面会直接恢复结果，不再重复请求上游；点击“刷新余额”会绕过缓存重新查询。相同账户的并发查询会合并，刷新失败也不会覆盖尚未过期的成功缓存。
