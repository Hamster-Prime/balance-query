...
func (s Sub2API) Fetch(authID, token string) balance.Result {
	baseURL := s.BaseURL
	if baseURL == "" {
		return errResult(authID, balance.ProviderLabel[balance.ProviderSub2API],
			"Sub2API base URL not configured — please set it in the Balance Query settings page")
	}
	var resp sub2APIUsageResp
	if err := getJSON(baseURL+"/v1/usage", token, &resp); err != nil {
	...
	display := ""
	if d.TokensTotal > 0 {
		display = fmt.Sprintf("%d / %d tokens 已用", d.TokensUsed, d.TokensTotal)
	} else if d.Quota > 0 {
		const u = 500000.0
		display = fmt.Sprintf("$%.4f 剩余 (已用 $%.4f)", d.Quota/u, d.UsedQuota/u)
	}
